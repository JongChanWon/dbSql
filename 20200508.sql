...
5] NOT NULL
    ...
    --컬럼 가운데는 추가가 불가능하다
NOT NULL 제약조건
:컬럼의 값이 NULL 이 들어오는 것을 방지하는 제약 조건
...
데이터 입력시(emp) 참조하는 테이블(dept)의 데이터 존재 유무를 빠르게 알기 위해서 
참조하는 테이블(dept) 의 컬럼(deptno) 에 인덱스가 반드시 생성 되어 있어야만
FOREIGN KEY 제약조건을 추가 할 수 있다.

UNIQUE 제약조건을 생성할 경우 해당 컬럼의 값 중복 체크를 빠르게 하기 위해서
오라클 에서는 해당 컬럼에 인덱스가 없을 경우 자동으로 생성한다

PRIMARY KEY 제약조건 : UNIQUE 제약 + NOT NULL 제약
...
98번 부서는 존재 하지 않으므로 정상적으로 입력 할 수 없다
INSERT INTO emp_test VALUES (9998, 'sally', 98);


FOREIGN KEY 제약조건 테이블 
...
CREATE TABLE emp_test (
    ...
    CONSTRAINT fk_emp_test_dept_test FOREIGN KEY (deptno) 컬럼 REFERENCES  dept_test (deptno)
    --CONSTRAINT 제약조건 명 제약조건TYPE 컬럼 REFERENCES  참조하는 테이블 (컬럼)
   
);
--index 는 객체생성을 할때 속도향상을 위해
ROLLBACK;
...
부모 레코드(dept)test.deptno) 를 참조하고 있는 자식 레코드(emp_test.deptno) 가
...
3. ON DELETE SET NULL ==> 부모가 삭제되면 참조하고 있는 자식 데이터를 NULL 로 설정
...
emp 테이블에서 급여정보 (sal) 를 관리할때 sal 컬럼의 값은 0보다 작은 값이 들어가는 것은
로직적으로 이상함.
...
*unique primary 키 제약 참조제약 index 가 자동 생성*


CTAS: Create Table AS
SELECT 결과를 이용하여 테이블을 생성하는 명령
NOT NULL 제약조건 제외한 다른 제약 조건은 복사되지 않는다
용도
  1. 개발자 레벨의 백업
  2. 개발자 레벨의 테스트
...
SELECT *
FROM dept_test
WHERE 1 != 1; -- 이구문은 조회가 되지 않는다 false 라서
...
이미 생성된 테이블을 수정 할 수도 있음
    1. 새로운 컬럼을 추가
      ...
      ***** 기존 생성되어 있는 컬럼 중간에는 새로운 컬럼을 추가할 수가 없다
        ==> 기존 테이블을 삭제하고 컬럼 순서를 조정한 새로운 테이블 생성 DDL 로
            새롭게 테이블을 만들어야 함
            운영중이라면 이미 데이터가 쌓여있을 가능성이 굉장히 높음
            ==> 테이블을 생성할 때 신중하게 컬럼 순서를 고려
    
    2. 기존 컬럼 삭제, 컬럼 이름 변경, 컬럼 사이즈 변경, (타입도 제한적으로 변경 가능)
       참조키가 걸려있는 테이블의 컬럼 이름을 변경 하더라도 참조하는 테이블에는 영향이 가지 않는다
       (알아서 이름을 변경해준다)
       EX) emp_test.deptno ==> dept_test.deptno 을 참조 하고있는데
           dept_test.deptno 이름변경하여 dept_test.dno 로 수정하더라도 fk는 변경된 이름으로
           잘 유지가 된다
...
ALTER TABLE emp_test MODIFY (hp DATE); --수정도 중간은 안된다
--타입 변경은 안되는데 안에 데이터가 없어서 변경이 됨
위에서 한 변경사항 확인
SELECT *
FROM emp_test;
...
ALTER TABLE emp_test MODIFY (hp VARCHAR2(30)); --데이터가 있으므로 수정 안됨
...
DDL 은 자동 커밋이기 때문에 DML 문장에도 영향을 받는다
