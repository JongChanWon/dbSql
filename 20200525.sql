...
고객한명당 제품을 2~3개 정도 ㅐㅇ음
일실적(한달 20일이 영업일) 8000~12000 건 정도의 데이터
cycle 테이블
cid, pid, day, cnt
...
1   100 2020/05/11  1   : 1번 고객이 100번 제품을 2020/05/11일(월요일) 1개 먹는다
1   100 2020/05/18  1   : 1번 고객이 100번 제품을 2020/05/18일(월요일) 1개 먹는다
1   100 2020/05/25  1   : 1번 고객이 100번 제품을 2020/05/25일(월요일) 1개 먹는다
SELECT *
FROM cycle;
...
BEGIN
    --행의 결과는 여러건인데 변수는 스칼라 변수 ==> 테이블 타입의 변수로 변경 해야한다
...
JAVA  에서도 반드시 처리를 해야하는 예외를 잡아서 해당 회사나 개발자가 정의한 예외로 바꾸어서 처리할 수 있다
mybatis 같은 ORM(Object Relation mapping) 프레임워크에서도 jdbc 프로그래밍에서 반드시 들어가야 하는
...
만약에 해당하는 데이터가 없을 때 NO_DATA_FOUND ==> NO_EMP 개발자 정의 예외로 치환하여 의미상으로 좀더 명확하게 표현
SELECT *
FROM emp
WHERE empno = :empno;
